...
def get_latest_model_path(folder):
    folder = Path(folder)
    pattern = re.compile(r"ep_(\d+)\.model$")
    ...
    for f in folder.iterdir():
        ...
        if match:
            ...
            models.append((epoch, f))
    
    if not models:
        return None  # no matching files
    
    # get file with highest epoch
    latest = max(models, key=lambda x: x[0])[1]
    return latest
...
def main(args):
    with torch.no_grad():
        ...
        if args.dataset == 'afhq':
            ...
            path_to_param = os.path.join(args.save_root, args.dataset, args.metric)
            path_to_param = get_latest_model_path(path_to_param)
            print(path_to_param)
            curvature_param = torch.load(path_to_param, weights_only=False)
            curvature_net.load_state_dict(curvature_param["weight"])
            curvature_net.eval()
    ...
    figure_path = os.path.join(args.figure_path, args.metric)
    ...
    with torch.no_grad():
        ...
        plot_img(x_t, nrow=z_test_0.size(0), ncol=len(reg_space), saving_path=figure_path, title=args.metric)

        # --- Prepare real images ---
        flt = torch.logical_or(d_loader.data_train['label'] == 0,
                            d_loader.data_train['label'] == 1)

        real_latents = d_loader.data_train['latent'][flt]  # select latents
        real_images = x_to_z(real_latents, inverse=True)  # decode to images

        # make sure the images are in [0,1] and correct shape
        real_images = real_images.view(-1, *im_size)
        real_images = (real_images.clamp(-1, 1) + 1) / 2
        fake_images = (x_t.clamp(-1, 1) + 1) / 2
        ...
        # --- FID/KID setup (do once outside ideally) ---
        fid = FrechetInceptionDistance(feature=2048).to(args.device)
        kid = KernelInceptionDistance(subset_size=len(fake_images)-1).to(args.device)

        # --- Update metrics ---
        for i in range(0, len(real_images), 1):
            batch = real_images[i:i+1]
            fid.update(batch, real=True)
        for i in range(0, len(fake_images), 1):
            batch = fake_images[i:i+1]
        ...
        txt_path = os.path.join(figure_path, "metrics.txt")

        with open(txt_path, "w") as f:
            f.write(f"Metric: {args.metric}\n")
            f.write(f"FID: {fid_score.item():.6f}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--figure_path', type=str, default='/work3/fmry/projects/RiemannEBM/figures/')
