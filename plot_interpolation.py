    ...
    for metric in metric_types:
        with torch.no_grad():
            ...
            if args.dataset == 'afhq':
                ...
                path_to_param = os.path.join(args.save_root, args.dataset, metric)
            ...
            if approximator == 'ebm':
                ...
                if h_function == "invp":
                    ...
                    raise NotImplementedError()
            elif metric == "newton_riemann_logp":
                path_to_param = os.path.join(args.ebm_root, args.ebm_ckpt)
                ebm_param = torch.load(path_to_param, weights_only=False)
                netE = ebm_param["type"](**ebm_param["args"]).to(args.device)
                netE.load_state_dict(ebm_param["weight"])
            ...
            elif metric == "newton_riemann_mlogp":
                path_to_param = os.path.join(args.ebm_root, args.ebm_ckpt)
                ebm_param = torch.load(path_to_param, weights_only=False)
                netE = ebm_param["type"](**ebm_param["args"]).to(args.device)
                netE.load_state_dict(ebm_param["weight"])
            ...
            elif metric == "newton_riemann_invp":
                path_to_param = os.path.join(args.ebm_root, args.ebm_ckpt)
                ebm_param = torch.load(path_to_param, weights_only=False)
                netE = ebm_param["type"](**ebm_param["args"]).to(args.device)
                netE.load_state_dict(ebm_param["weight"])
            ...
            elif metric == "newton_riemann_minvp":
                path_to_param = os.path.join(args.ebm_root, args.ebm_ckpt)
                ebm_param = torch.load(path_to_param, weights_only=False)
                netE = ebm_param["type"](**ebm_param["args"]).to(args.device)
                netE.load_state_dict(ebm_param["weight"])
                ...
                raise NotImplementedError()
            print(f"{metric} -- min={args.min_h} -- max={args.max_h}")
        ...
        with torch.no_grad():
            ...
            image_x_t.append(x_t)

    stacked = torch.stack(image_x_t)
    reordered = stacked.transpose(0, 1)

    for i, img_row in enumerate(reordered, start=0):
        figure_path = os.path.join(args.figure_path, 'comparison_plot')
        os.makedirs(figure_path, exist_ok=True)
        plot_img(img_row, nrow=len(metric_types), ncol=len(reg_space), saving_path=figure_path, title=f'index_{i}')
